...
def kthAlternate(head,k):
    ...
    while count < k and curr is not None: # logic of reversing a LL
        next=curr.next
        curr.next=prev
        prev=curr
        curr=next
    ...
    if head is not None: #After reversing the Kth element becomes the head, and this is being done on the parent LL.
        head.next=curr
    count=0
    while count<k-1 and curr is not None: #Traversing the non k element
        curr=curr.next
        count=count+1

    if curr is not None:
        curr.next=kthAlternate(curr, k)
    return prev
